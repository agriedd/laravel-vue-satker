        ...
        <v-dialog v-model="dialog" content-class="shadow fill-height rounded-0 ma-0 ml-auto h-100" scrollable max-width="400" persistent transition="slide-x-reverse-transition">
            <v-form @submit.prevent="submit($event)" :disabled="loading">
                <v-card rounded="0">
                    ...
                    <v-card-text>
                        <form-tambah-kegiatan v-if="dialog"></form-tambah-kegiatan>
                    </v-card-text>
                    <div>
                        <v-divider/>
                        <v-card-actions>
...
import FormTambahKegiatan from './FormTambahKegiatan.vue'
export default {
    components: {
        FormTambahKegiatan,
    },
    ...
    computed: {
        ...mapState({
            modal_tambah: state => state.kegiatan.modal.tambah,
        }),
        dialog: {
            set(val){
