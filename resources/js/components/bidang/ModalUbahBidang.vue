        ...
        <v-dialog v-model="dialog" content-class="shadow fill-height rounded-0 ma-0 ml-auto h-100" scrollable max-width="400" persistent transition="slide-x-reverse-transition">
            <v-form @submit.prevent="submit($event)" :disabled="loading">
                <v-card rounded="0">
                    <div>
                        <v-card-text>
                            Form Ubah Data Bidang
                        </v-card-text>
                        <v-divider/>
                    </div>
                    <v-card-text>
                        <form-ubah-bidang v-if="dialog" @loading="loading = $event"/>
                    </v-card-text>
                    <div>
                        <v-divider/>
                        <v-card-actions>
...
import FormUbahBidang from './FormUbahBidang.vue'
export default {
    components: {
        FormUbahBidang,
    },
    ...
    computed: {
        ...mapState({
            modal_ubah: state => state.bidang.modal.ubah,
        }),
        dialog: {
            set(val){
