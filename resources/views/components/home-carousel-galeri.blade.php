...
<div class="grid-carousel-galeri h-100">
    @foreach ($arr as $key => $item)
        @if($key != 4)
            <div class="h-100 {{ $item }} bg-white p-lg-4 pt-lg-5 position-relative" style="background-image: url('{{ Storage::disk('public')->url(optional($galeri->get($key))->gambar) }}'); background-size: cover; background-position: center">
                <div class="position-absolute h-100 w-100" style="background: rgba(0, 0, 0, 0); top: 0; left: 0;"></div>
            </div>
        @else
            <div class="h-100 grid-carousel-galeri--item-4 bg-dark d-flex flex-column justify-content-center" style="background-image: url('{{ Storage::disk('public')->url(optional($galeri->get($key))->gambar) }}'); background-size: cover; background-position: center">
